...
def create_project_pdf(project, filename):
    c = canvas.Canvas(filename, pagesize=letter)
    
    # Set up the page
    c.setFont("Helvetica", 12)
    
    # Add a title with project information
    c.setFont("Helvetica-Bold", 16)
    c.drawCentredString(letter[0]/2, letter[1] - inch, f"Project Report: {project.name}")
    ...
    project_details = [
        ...
        f"Name: {project.name}",
        f"Created: {project.created_at if hasattr(project, 'created_at') else 'N/A'}",
        "",
        "Report Contents:",
        "- Project Overview",
        "- Key Metrics",
    ...
    # Add a colored rectangle for visual separation
    c.setFillColor(colors.blue)
    c.rect(inch, y - 0.5 * inch, 2 * inch, 0.5 * inch, fill=1)
    
    # Add a footer
    c.setFont("Helvetica-Oblique", 10)
    c.setFillColor(colors.gray)
    c.drawCentredString(letter[0]/2, 0.5 * inch, "Generated by ReportLab")
